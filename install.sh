...
download_dot_from_git() {
  ...
  command git --git-dir="${DOWNLOAD_DIR}/.git" remote add origin "$dot_source" \
  ...
  command git -c advice.detachedHead=false --git-dir="$DOWNLOAD_DIR"/.git \
    --work-tree="$DOWNLOAD_DIR" checkout -f --quiet "$dot_latest_version"
